...
class attnLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory_list, tgt_mask=None, memory_mask=None,
        ...
        for memory, multihead_attn, norm2, dropout2, m_pos in zip(memory_list, self.multihead_attn_list,
            ...
            tgt2 = \
            multihead_attn(query=self.with_pos_embed(tgt, pos), key=self.with_pos_embed(memory, m_pos), value=memory,
...
class TransDecoder(nn.Module):
    def __init__(self, num_attn_layers, device='cuda:1', hidden_dim=128):
        super(TransDecoder, self).__init__()
        attn_layer = attnLayer(hidden_dim)
        self.layers = _get_clones(attn_layer, num_attn_layers)
        self.position_embedding = build_position_encoding(device, hidden_dim)
        self.device = device

    def forward(self, imgf, query_embed):
        pos = self.position_embedding(torch.ones(imgf.shape[0], imgf.shape[2], imgf.shape[3]).bool().to(
            self.device))  # torch.Size([1, 128, 36, 36])
...
class GeoTr(nn.Module):
    def __init__(self, device):
        ...
        for i in self.encoder_block:
            self.__setattr__(i, TransEncoder(2, hidden_dim=hdim, device=device))
        ...
        for i in self.decoder_block:
            self.__setattr__(i, TransDecoder(2, hidden_dim=hdim, device=device))
